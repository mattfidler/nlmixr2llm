...
`nlmixr2llm` ships LLM-facing documentation for the nlmixr2 pharmacometric modeling ecosystem, organized around the tasks a pharmacometrician performs rather than around individual packages. It bundles a single combined `nlmixr2verse` agent (the orchestration layer) plus one skill per task for on-demand depth:
...
| `design` | Optimal design: evaluating and optimizing a planned study | babelmixr2, PopED |
...
`list_agents()` returns the single combined `nlmixr2verse` agent; `list_tasks()` (equivalently `list_skills()`) enumerates the four task skills, and `list_packages()` reports which nlmixr2-universe packages those skills cover. You can read any single document directly:
...
A skill is a directory, not a single file: `SKILL.md` carries the compact guidance and `references/*.md` add depth an agent can pull in on demand.
...
By default `system_prompt()` includes the agent and the skills for all tasks. Subset with `tasks = ...` or `include = c("agents", "skills")`, and add `references = TRUE` to append each skill's supporting reference files.
...
Codex enforces a default 32 KiB cap on combined `AGENTS.md` content (`project_doc_max_bytes` in `~/.codex/config.toml`). The agent plus all five skills is ~51 KiB, above the cap. The `nlmixr2verse` agent is ~10 KiB and is always included whole when agents are requested; `tasks = ...` subsets the skills (~7-10 KiB each). The agent plus any two skills fits (up to ~29 KiB; a package test enforces this), so pick the tasks you actually need:

```{r codex_subset, eval = FALSE}
# Agent plus the two most common tasks (~26 KiB); three tasks do not fit
install_codex(scope = "project", tasks = c("simulation", "estimation"))

# Just the agent (~10 KiB) -- routing and conventions only
...
`install_claude_code()` and `install_positron(style = "instructions")` record what they wrote in a manifest (`.nlmixr2llm-manifest`) in the install location. On re-install they **prune** files this package installed in an earlier version but no longer ships — for example, upgrading from the per-package skills of 0.1.0 (`rxode2`, `nlmixr2`, ...) to the task skills removes the old directories rather than leaving them behind as duplicates. Pruning is on by default (`prune = TRUE`) and only ever touches files nlmixr2llm itself created; your own agents and skills are never removed. Selecting a subset with `tasks = ...` does not prune the skills of tasks you leave out — only content the current version no longer ships at all is removed.
